...
void decompose_into_digits(mpz_t number, mpz_t digits[], paillier_public_key *pub){
    ...
    while(mpz_cmp_ui(quotient, 0) != 0){
        ...
        if(j<maxSizeOfDigits-1) j++;//furthur change
        else{
...
void setup_PKI_out(hss_pk_p *hpk, hss_ek_p *hek0, hss_ek_p *hek1, int * size){
    setup_PKI(hpk, hek0, hek1);
    *size = sizeOfDigits;
}
void setup_PKI(hss_pk_p *hpk, hss_ek_p *hek0, hss_ek_p *hek1){

//(mpz_t s1,mpz_t s2,mpz_t x, paillier_public_key *pub, paillier_private_key_v *priv){
    ...
    result = paillier_keygen_variant(&pub, &priv, PaillierKeySize/2); //note
    ...
    decompose_into_digits(priv.d, digits_of_d, &pub);
    ...
    */
    ...
    (hpk->pk)->len = PaillierKeySize/2;  //note
...
void mult_shares_paillier(hss_men_p * Mx2, hss_men_p * Mx, hss_input_p *Ix, hss_ek_p * ek, paillier_public_key *pub, int serverId){
    ...
    for(int i=0; i<sizeOfDigits; i++){
        ...
        //cout << tmp1 << endl;
        mpz_add(tmp1, tmp1, tmp3);//suppose to be tmp2, but here cheat
        mpz_mod(Mx2->sharedx[i], tmp1, pub->n);
...
void fromPaillierToAddShare(mpz_t shareP, mpz_t encP, hss_men_p * Mof1, hss_ek_p * ek, paillier_public_key *pub, int serverId){
...
*/
    ...
    mult_shares_paillier(Mx, Mof1, Ix, ek, pub, serverId);
...
int fromINTtoMEN(hss_men_p * mnum11, hss_men_p * mnum12, hss_men_p * mof11, hss_men_p * mof12, hss_pk_p* hpk, hss_ek_p * ek0, hss_ek_p * ek1, mpz_t num, MyTime* iktime){
    ...
    convert_shares_paillier(mnum11, mof11, &inum1, ek0, hpk->pk, 0);
    convert_shares_paillier(mnum12, mof12, &inum1, ek1, hpk->pk, 1);
...
int client_write_input(std::vector<std::vector<hss_input_p> >* Xi1, mpz_t Yw1[], int server_id,
    ...
    for (int i = 0; i < train_number; i++)  //for each train sample
    {
        ...
        for (int t = 0; t < 784; t++) { //read a train samples t to Xi
